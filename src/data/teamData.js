export const teamMembers = [
  {
    id: 'carlos-olaechea',
    name: 'Dr. Carlos Fernández',
    role: 'Socio Fundador y Director Legal',
    exp: '25 Años de Trayectoria',
    img: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=600&auto=format&fit=crop',
    bio: 'Abogado por la Universidad Nacional de Trujillo, Máster en Derecho Civil y procesal. Reconocido por su rigurosidad técnica, liderazgo en patrocinio procesal y compromiso inquebrantable en la defensa de causas complejas en La Libertad.',
    registry: 'Colegio de Abogados de La Libertad — Reg. CALL 4521',
    recognitions: [
      'Premio a la Excelencia Jurídica — Colegio de Abogados de La Libertad, 2020',
      'Ex-Magistrado del Poder Judicial de la Libertad',
      'Miembro de la Sociedad Peruana de Derecho Procesal'
    ]
  },
  {
    id: 'mariana-rios',
    name: 'Dra. Mariana Ríos',
    role: 'Socia — Especialista en Derecho Penal',
    exp: '15 Años de Experiencia',
    img: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=600&auto=format&fit=crop'
  },
  {
    id: 'fernando-vargas',
    name: 'Dr. Fernando Vargas',
    role: 'Especialista en Derecho Corporativo',
    exp: '12 Años de Experiencia',
    img: 'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=600&auto=format&fit=crop'
  },
  {
    id: 'elena-torres',
    name: 'Dra. Elena Torres',
    role: 'Asesora en Derecho Laboral',
    exp: '10 Años de Experiencia',
    img: 'https://images.unsplash.com/photo-1580489944761-15a19d654956?q=80&w=600&auto=format&fit=crop'
  },
  {
    id: 'luis-mendoza',
    name: 'Dr. Luis Mendoza',
    role: 'Asistente Legal y Litigación',
    exp: '5 Años de Experiencia',
    img: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=600&auto=format&fit=crop'
  },
  {
    id: 'patricia-castro',
    name: 'Lic. Patricia Castro',
    role: 'Coordinadora de Atención al Cliente',
    exp: '7 Años de Experiencia',
    img: 'https://images.unsplash.com/photo-1598550874175-4d0ef436c909?q=80&w=600&auto=format&fit=crop'
  }
];
