...
1. **Botón Menú Hamburguesa Móvil (Header.css)**:
   - En vistas móviles (`@media (max-width: 992px)`), el contenedor `.header-container` debe tener `padding: 0 1.5rem;`, el logo `.logo-img` debe reducir su alto a `height: 46px;` para dejar espacio libre, y el botón `.mobile-menu-btn` debe llevar `margin-right: 0;`, `padding: 0.5rem;` y `flex-shrink: 0;`. Esto posiciona al icono a exactamente 1.5rem del borde derecho sin desbordar hacia afuera.
